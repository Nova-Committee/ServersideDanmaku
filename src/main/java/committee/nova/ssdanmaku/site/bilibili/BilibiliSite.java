package committee.nova.ssdanmaku.site.bilibili;
...
import committee.nova.ssdanmaku.event.post.UpdatePopularInfoEvent;
import committee.nova.ssdanmaku.site.ISite;
import committee.nova.ssdanmaku.utils.BilibiliMsgSplit;
import committee.nova.ssdanmaku.utils.Zlib;
...
public class BilibiliSite implements ISite {
    ...
    public void initMessage(WebSocketClient client) {
        long id = RoomId.getRealRoomId(config.getRoom().getId());
        if (id == -1) {
    ...
    private void handJsonMessage(ByteBuf data) {
        ...
        ServersideDanmaku.LOGGER.info(message);
    }
